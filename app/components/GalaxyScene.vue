...
const generateGalaxy = () => {
    ...
    const nebulaColors = [
        new THREE.Color("#b48cff"), new THREE.Color("#d48cff"),
        new THREE.Color("#7fa6ff"), new THREE.Color("#ff9fb3"),
        new THREE.Color("#6b7fd7"), new THREE.Color("#ffd6a5"),
    ];
    ...
    for (let i = 0; i < params.count; i++) {
        ...
        randomnessArray[i3] = Math.pow(Math.random(), params.randomnessPower) * (Math.random() < 0.5 ? 1 : -1) * params.randomness * radius;
        randomnessArray[i3 + 1] = Math.pow(Math.random(), params.randomnessPower) * (Math.random() < 0.5 ? 1 : -1) * params.randomness * (radius * 0.5);
        randomnessArray[i3 + 2] = Math.pow(Math.random(), params.randomnessPower) * (Math.random() < 0.5 ? 1 : -1) * params.randomness * radius;
        ...
        if (ratio < 0.2) {
            ...
            tempColor.copy(colorInside).lerp(colorOutside, (ratio - 0.2) * 1.25);
        }
